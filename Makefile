...
E2E_TEST_PATTERN := ^(TestNewSessionQuestionPersistsToolEvidenceAndDiagnosis|TestSessionApplicationAdapterUsesRealSQLiteResumeEligibility|TestResumeIntegrationUsesTemporaryDatabaseAndRevalidatesOnlyAcceptedScope|TestDiagnosisScenarioFixtures|TestCoordinator.*(Retention|Privacy|Minimal|Delete|Export).*|TestApprovalCoordinator.*SessionDeletion.*|TestExportSummary.*|TestSessionRepositoryExportSnapshot.*|TestExportWriter.*|TestSessionRepositoryDeletionCascades.*|TestRetentionRepository.*|TestPrivacyExport.*|TestPrivacyDoesNotOfferExportForMinimalPersistence|TestPrivacyLifecycleControlsReuseOneComposerAndRequireDeleteConfirmation|TestResumePickerDeletesOnlyAfterExplicitConfirmation|TestTopLevelResumePickerDeletionNeverFallsBackToANewSession|TestRestartApprovalEndToEndWriteActionMatrix)$$
