...
MIGRATION_TEST_PATTERN := ^(TestMigrate.*|TestInitialSchema.*|TestActionAuthoritySchema.*)$
...
.PHONY: bootstrap-tools release-tools go-version-check release-version-check release-config-check release-cgo-check release-dry-run release-verify fmt imports fmt-check imports-check lint workflow-lint test test-race test-security security test-migration test-e2e test-fuzz-seeds test-performance test-integration-contracts test-integration-model test-integration-model-preferred test-integration-session test-integration-kubernetes test-reviewer-eval-offline test-reviewer-eval-live test-integration import-guard dependency-guard vuln vet build cross-build binary-size-check platform-smoke check check-slow check-all
...
release-version-check:
	@test "$(RELEASE_VERSION)" = '0.1.0' || { \
		printf '%s\n' 'RELEASE_VERSION must remain 0.1.0 until the first publication.'; \
		exit 1; \
	}
