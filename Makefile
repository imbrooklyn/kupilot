...
SECURITY_TEST_PATTERN := ^(TestSecurityAssurance.*|TestRedactor.*|TestStreamingRedactor.*|TestOutputGuard.*|TestScopeManager.*|TestSlashRegistryIsFixedAndReadOnly|TestParseSlashDraftRules|TestValidateModelEndpointPolicy|TestRedirectPolicyAllowsOnlyCanonicalOrigin|TestModelClient.*|TestCollectModelMessage.*|TestAdapter(BlocksHighRiskModelTextBeforeDownstreamAction|DoesNotInheritCallerEinoCallbacks|RejectsHostileToolSelectionsBeforeHandler|DiscardsCommentaryAccompanyingToolSelection|BlocksJSONEscapedCredentialFromEveryDiagnosisSink|StopsStreamingWhenScopeBecomesStale|CancellationAfterProvisionalAnswerStopsOwnedStream|TimeoutAfterProvisionalAnswerPublishesNoDiagnosis|MalformedStreamAfterProvisionalAnswerPublishesNoDiagnosis|LengthLimitReplacesProvisionalAnswerWithLocalDiagnosis|StreamsFragmentedAnswerWithoutEnvelopeMetadata|ExecutesExactArgvWithMinimalEnvironmentAndSanitizedOrderedOutput|RejectsExecutableReplacementAndSymlinkWorkingDirectoryBeforeStart|RejectsScriptExecutableBeforeStart|BoundsCombinedStdoutStderrAndBlocksSensitiveOutput|CancellationKillsProcessGroupAndBoundsJoin|TimeoutIsUnknownAndNeverRetries)|TestProvisionalEventBudgetIsSharedByTheEntireRun|TestMaliciousToolOutputCannotAuthorizeAnotherTool|TestFileLoggerWritesBoundedSafeModelFailureDiagnostics|TestFileLoggerMarksSafeModelCallStackTruncation|TestFileLoggerWritesOnlyExplicitOptInSensitiveModelDiagnostics|TestFileLoggerBoundsOptInSensitiveFields|TestSessionPermissionRuleMatchesOnlyOneExactLocalArgv|TestS04ExecutionPermissionRouteMatrixIsComplete|TestAuditEventAllowsOnlyDigestBoundCrossNamespaceDrainPhases|TestAuditRepositoryRoundTripsDigestBoundCrossNamespaceDrainPhase|TestDatabaseAndWALExcludeProhibitedContentCanaries|TestDiagnosisRepositoryUsesAllAcceptedEvidenceForObservationWindow|TestSanitizeExternalText.*|TestApplicationTextAndScopeAreSanitizedBeforeRenderState|TestToolCallBindingSanitizesOrBlocksModelFreeTextBeforeHandler|TestDiagnosisValidatorSanitizesEveryModelFreeTextField|TestDiagnosisValidatorBlocksHighRiskModelTextWithoutSealingEvidence|TestNewSessionQuestionPersistsToolEvidenceAndDiagnosis|TestPrivacy(ConsentLifecycleBindsOriginCategoriesAndPolicy|ManagerDefersStoreIOUntilModelOriginIsConfigured)|TestModelSetupSecret.*|TestModelAPIKeyCanaryIsAbsentFromEveryStartupSink|TestFileModelAPIKeyCanaryIsExtractedFromOrdinaryConfiguration|TestLoadEnvironmentCredentialOverridesFileAndIsUnsetOnce|TestSaveModelProfile.*|TestEnsureHome.*|TestPublishConfiguration.*|TestClearCache.*|TestCoordinator(UnconfiguredModel|ModelSetup).*|TestUnconfiguredModelSetup.*|TestUnconfiguredModelDoesNotPreemptExplicitResumeStartup|TestModelSlashReconfigures.*|TestInvalidModelCredentialKeepsComposerMaskedAndOutOfHistory|TestOptionalModelSetupCanBeCancelledWithoutChangingRuntime|TestCompositionRootCacheClear.*|TestApplicationRequestFilterAndDrainDestroyRejectedModelSecrets|TestObservation(PreflightDenialsPerformNoApprovalOrTargetRevalidation|TargetDenialsPerformZeroPodGets)|TestPodLog(ActionGateDenialsPreventUnsupervisedReads|OutcomeAuditFailureSuppressesSanitizedContent)|TestDataSource(ActionGateDenialsPreventUnsupervisedNetworkCalls|OutcomeAuditFailureSuppressesReturnedContent)|TestApprovalRepositoryStoresOnlyObservationParameterDigest|Test(ResolvePaths|CanonicalizeExistingHome|SystemPaths).*|TestOpenRespectsExistingUserModesAndConfiguresConnectionPragmas|TestOpenCreatesPrivateStateAndDatabase|TestFileLoggerCreatesOwnerOnlyDirectoryAndFile|TestFileLoggerHandlesUserManagedModesAndRejectsUnsafePaths|Test(RemoteDiagnostics|RemoteDiagnostic|PodExec|ContainerFile|DiagnosticPod|RunDiagnosticPod|LoadRemoteDiagnostics|BindRemoteDiagnostics|BindContainerFile|RemoteToolSchemas|LocalCommand|LocalEnvironment|LocalExecution|ShellCommand|LoadLocalExecution|SharedDispatcher|LocalOutcomeAudit|LocalDispatcher|ReadOnlyDeniesLocalExecution|CoordinatorBridgesEveryTypedProposal|CoordinatorScalePreflight|CoordinatorReadOnlyProposal|Remediator|Drain|RollbackPreparation|RollbackVerification|TypedRemediation|RemediationAttempt|ScaleCancellation).*|Test(Conversation|QueuedFollowUp|PendingSteer|PendingAndUnknownSteers|PopLastConversationInput|ActiveConversation|ActiveTabKeeps|CommittedEscapedSteer|Steer|MultipleSteers|SummarizationCompletesBeforeSteer|SummaryFailureLeavesSteer).*)$$
MIGRATION_TEST_PATTERN := ^(TestMigrate.*|TestReleasedMigrationMatrix.*|TestApprovalRuntimeMigration.*|TestMinimalRunIdentityMigration.*|TestRunConversationSequenceMigration.*|TestCompatibleBinaryUpgradePreservesSessionListingAndResumableHistory|TestMigrationTenAdmitsCompleteToolCatalog|TestInitialSchemaContainsOnlyAllowlistedStorageColumns|TestActionAuthoritySchemaContainsNoRawParameterOrTransportColumns|TestActionAuthoritySchemaChecksMirrorDomainShape)$$
E2E_TEST_PATTERN := ^(TestNewSessionQuestionPersistsToolEvidenceAndDiagnosis|TestSessionApplicationAdapterUsesRealSQLiteResumeEligibility|TestResumeIntegrationSeparatesExplicitScopeActivationFromZeroIOAcceptance|TestDiagnosisScenarioFixtures|TestCoordinator.*(Retention|Privacy|Minimal|Delete|ClearHistory|Export).*|TestApprovalCoordinator.*(SessionDeletion|HistoryDeletion).*|TestCoordinatorRepositoryClearHistory.*|TestDeleteAllLocalState.*|TestExportSummary.*|TestSessionRepositoryExportSnapshot.*|TestExportWriter.*|TestSessionRepositoryDeletionCascades.*|TestRetentionRepository.*|TestPrivacyExport.*|TestPrivacyDoesNotOfferExportForMinimalPersistence|TestPrivacyLifecycleControlsReuseOneComposerAndRequireDeleteConfirmation|TestPrivacyClearHistoryAndDeleteAllRequireExplicitConfirmation|TestResumePickerDeletesOnlyAfterExplicitConfirmation|TestTopLevelResumePickerDeletionNeverFallsBackToANewSession|TestRestartApprovalEndToEndWriteActionMatrix|TestSharedDispatcherExecutesEveryTypedRemediationOnlyAfterDurableConsume|TestCoordinatorBridgesEveryTypedProposalToFreshPreparationAndApproval|TestLocalDispatcherApprovalLifecycleMakesAtMostOneProcessAttempt|TestObservationHumanApprovalReleasesOneConsumedEnvelopeAndContentFreeOutcome)$$
FUZZ_SEED_PATTERN := ^(FuzzRedactorSafety|FuzzStreamingRedactorSafety|FuzzBindToolCallStrictSchema|FuzzBoundedSSEBodyIsChunkIndependent|FuzzParseSlashDraftHasNoDynamicAuthority)$$
IMPORT_GUARD_PATTERN := ^(TestApplicationImportBoundaryIsStatic|TestDomainDoesNotReintroduceNeutralModelProtocolDTOs|TestEinoImportsRemainInTheSoleTranslationBoundary|TestExportedAgentAdapterBoundaryContainsNoEinoTypes|TestProductionUsesOneADKRuntimeAndNoParallelMemoryFramework|TestExportedKubeBoundaryContainsNoClientGoTypes|TestReadOnlyToolPathContainsNoWriteShellOrGenericKubernetesEscape|TestTUIImportBoundaryAndSingleTextareaAreStatic|TestRepositorySourcesKeepExplicitSQLBoundary|TestSQLXImportRemainsInsideSQLiteAdapter|TestCompositionConstructsOneModelLifecycleAndOneSupervisedActionPath|TestSelectedDriverAndSQLXContract|TestProductionHasClosedSupervisedKubernetesExecutors)$$

.PHONY: bootstrap-tools release-tools go-version-check release-version-check release-config-check release-cgo-check release-dry-run release-verify fmt imports fmt-check imports-check lint workflow-lint test test-race test-security security test-migration test-e2e test-fuzz-seeds test-performance test-integration-contracts test-integration-model test-integration-model-preferred test-integration-model-ollama test-integration-session test-integration-kubernetes test-reviewer-eval-offline test-reviewer-eval-live test-integration import-guard dependency-guard vuln vet build cross-build binary-size-check platform-smoke check check-slow check-all
...
# Tagged integration is intentionally absent from check/check-slow and hosted
# CI. Live cases remain blocked unless the caller supplies every explicit gate.
