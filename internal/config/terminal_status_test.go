...
func TestTerminalStatusTitlesDefaultAndExplicitDisable(t *testing.T) {
	...
	if Defaults().ReducedMotion {
		t.Fatal("reduced motion must remain an explicit opt-in")
	}
	root := t.TempDir()
...
func TestReducedMotionLoadsAndIsPreservedByProfileWrite(t *testing.T) {
	...
	if !loaded.ReducedMotion {
		t.Fatal("explicit reduced motion was ignored")
	}

	secret, err := NewSecretValue("reduced-motion-test-key")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(content), "reduced_motion: true") {
		t.Fatalf("saved configuration omitted reduced-motion policy: %s", content)
	}
}
