...
func TestValidateConfigurationFields(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "reasoning effort", mutate: func(c *Config) { c.Models.Agent.ReasoningEffort = "unknown" }, code: "config_reasoning_effort_invalid"},
		{name: "response format", mutate: func(c *Config) { c.Models.Agent.ResponseFormat = "automatic" }, code: "config_response_format_invalid"},
		{name: "temperature below zero", mutate: func(c *Config) { c.Models.Agent.Temperature = testTemperature(-0.01) }, code: "config_temperature_invalid"},
		{name: "temperature above maximum", mutate: func(c *Config) { c.Models.Agent.Temperature = testTemperature(0.21) }, code: "config_temperature_invalid"},
...
func TestLoadInvalidEnvironmentDoesNotEchoValue(t *testing.T) {
	...
	_, err := Load(context.Background(), LoadOptions{
		...
		LookupEnv: lookupMap(map[string]string{
			"KUPILOT_AGENT_REQUEST_TIMEOUT_SECONDS": canary,
		}),
	})
