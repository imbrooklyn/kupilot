...
const (
	getResourceSchema         = `{"additionalProperties":false,"properties":{"detail":{"enum":["summary","diagnostic"],"type":"string"},"purpose":{"maxLength":1024,"minLength":1,"type":"string"},"resource":{"additionalProperties":false,"properties":{"api_version":{"enum":["v1","apps/v1","batch/v1"],"type":"string"},"kind":{"enum":["Pod","Deployment","ReplicaSet","Job","Service"],"type":"string"},"name":{"maxLength":253,"minLength":1,"type":"string"}},"required":["kind","name"],"type":"object"}},"required":["purpose","resource"],"type":"object"}`
	listResourcesSchema       = `{"additionalProperties":false,"properties":{"health_filter":{"enum":["any","abnormal"],"type":"string"},"kind":{"enum":["Pod","Deployment","ReplicaSet","Job","Service"],"type":"string"},"limit":{"maximum":50,"minimum":1,"type":"integer"},"name_query":{"maxLength":128,"type":"string"},"purpose":{"maxLength":1024,"minLength":1,"type":"string"}},"required":["kind","purpose"],"type":"object"}`
	getEventsSchema           = `{"additionalProperties":false,"properties":{"limit":{"maximum":50,"minimum":1,"type":"integer"},"purpose":{"maxLength":1024,"minLength":1,"type":"string"},"resource":{"additionalProperties":false,"properties":{"api_version":{"enum":["v1","apps/v1","batch/v1"],"type":"string"},"kind":{"enum":["Pod","Deployment","ReplicaSet","Job","Service"],"type":"string"},"name":{"maxLength":253,"minLength":1,"type":"string"},"uid":{"maxLength":256,"type":"string"}},"required":["kind","name"],"type":"object"},"since_seconds":{"maximum":86400,"minimum":60,"type":"integer"}},"required":["purpose","resource"],"type":"object"}`
	getPodLogsSchema          = `{"additionalProperties":false,"properties":{"container":{"maxLength":253,"minLength":1,"type":"string"},"pod_name":{"maxLength":253,"minLength":1,"type":"string"},"purpose":{"maxLength":1024,"minLength":1,"type":"string"},"since_seconds":{"maximum":3600,"minimum":60,"type":"integer"},"tail_lines":{"maximum":200,"minimum":1,"type":"integer"}},"required":["pod_name","purpose"],"type":"object"}`
	getRelatedResourcesSchema = `{"additionalProperties":false,"properties":{"include":{"items":{"enum":["owners","pods","replica_sets","service_endpoints","services"],"type":"string"},"maxItems":3,"minItems":1,"type":"array","uniqueItems":true},"purpose":{"maxLength":1024,"minLength":1,"type":"string"},"relation_depth":{"maximum":2,"minimum":1,"type":"integer"},"resource":{"additionalProperties":false,"properties":{"api_version":{"enum":["v1","apps/v1","batch/v1"],"type":"string"},"kind":{"enum":["Pod","Deployment","ReplicaSet","Job","Service"],"type":"string"},"name":{"maxLength":253,"minLength":1,"type":"string"},"uid":{"maxLength":256,"type":"string"}},"required":["kind","name"],"type":"object"}},"required":["purpose","resource"],"type":"object"}`
)
...
func ToolSpecifications() []domain.ModelToolSpecification {
	return []domain.ModelToolSpecification{
		...
		{Name: domain.ToolNameListResources, Version: ToolCatalogVersion, Description: "Find a bounded set of allowlisted resource candidates in the active Namespace; this is not a resource browser.", InputSchemaJSON: listResourcesSchema},
