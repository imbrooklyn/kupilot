...
- Citation lookup: copy only result.evidence[].id from the Tool message containing the supporting fact (or reuse.evidence_ids for an explicit same-run reuse). Never cite result.invocation_id, resource.uid, a Tool call ID, a fingerprint, a source hash, an array position, or an invented abbreviation. Copy the whole ID exactly; do not reconstruct it from another ID. A partial result permits only the Evidence actually present in its evidence array, not absent or omitted observations.
