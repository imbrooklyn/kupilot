...
func TestSystemPromptDoesNotEmbedQuestionOrToolLanguageInjection(t *testing.T) {
	...
	for _, required := range []string{
		...
		"a confirmed blocker does not rule out faults in untested layers",
		"a positive progress condition alone does not prove rollout completion",
		"distinguish observed readiness from untested application behavior",
		"Attribute user-reported checks to the user",
		"Preserve user-supplied resource names literally",
		"A different name is only a candidate, even if it is the single closest match",
		"stop detailed investigation",
		"Proceed directly for an exact unambiguous name",
		"Use typed needs_user_input only before any Tool read",
		"a confirmation request in answer_markdown; keep questions []",
		"Unavailable logs limit log-derived conclusions only",
		"An empty query, NotFound, or denied read may return no Evidence",
		"Return exactly one bare JSON object",
		"without a code fence, commentary, or trailing text",
		"Put answer_markdown first to enable provisional display",
		"object member order does not affect final validity",
		"answer_markdown",
		"evidence_citations",
		"proposed_actions",
