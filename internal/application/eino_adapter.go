...
// EinoConfig contains the complete project-owned inputs for the sole Eino runtime
...
// EinoRuntime runs one bounded ReAct composition at a time. All policy-bearing
...
var _ runExecutor = (*EinoRuntime)(nil)

// NewEinoRuntime constructs one native Eino runtime without performing model
// or Tool I/O.
func NewEinoRuntime(config EinoConfig) (*EinoRuntime, error) {
...
func newEinoRuntime(config runtimeConfig, client *modelClient) (*EinoRuntime, error) {
	if config.profileName == "" {
		config.profileName = string(domain.ModelRoleAgent)
	}
	if client == nil || (client.model == nil && client.responsesModel == nil) || config.scopeGuard == nil || config.identifiers == nil || config.now == nil ||
...
func (adapter *EinoRuntime) Run(ctx context.Context, input agent.RunInput, sink agent.EventSink) agent.RunOutcome {
	...
	if state.client.responsesModel != nil {
		finalMessage, err = state.runResponsesAgent(runCtx, initialMessages)
	} else {
		finalMessage, err = state.runChatAgent(runCtx, initialMessages)
	}
	if err != nil {
		return state.finishFailure(runCtx, err)
	}
...
func (adapter *EinoRuntime) Close() {
	if adapter == nil {
		return
	}
	adapter.lifecycleMu.Lock()
...
func normalizeFrameworkError(err error) error {
	...
	if errors.Is(err, adk.ErrExceedMaxIterations) {
		return localRuntimeStop(agent.RunStopModelCallLimit, domain.SafeErrorClassBudgetExhausted, "The run reached its fixed model-call limit.", err)
	}
...
// ModelName returns the explicitly configured model identifier.
func (runtime *EinoRuntime) ModelName() string { return runtime.client.configuration.Model }

// Origin returns the configured canonical origin.
func (runtime *EinoRuntime) Origin() string { return runtime.client.configuration.Origin }

// ProfileName returns the fixed Agent profile name.
func (runtime *EinoRuntime) ProfileName() string { return runtime.profileName }

func (state *runState) runChatAgent(runCtx context.Context, initialMessages []*schema.Message) (*schema.Message, error) {
	...
	for i, tool := range tools {
		infos[i], err = tool.Info(runCtx)
		if err != nil {
			return nil, err
		}
	}
	model, err := state.client.withTools(infos)
	if err != nil {
		return nil, err
	}
	...
	productionAgent, err := adk.NewChatModelAgent(runCtx, &adk.ChatModelAgentConfig{
		...
		MaxIterations: state.input.BudgetLimits().ModelCalls,
	...
	for {
		...
		if event.Action != nil || event.Output == nil || event.Output.MessageOutput == nil || event.Output.CustomizedOutput != nil {
			return nil, normalizeFrameworkError(errors.New("Eino Runner returned an unsupported event"))
		}
		message, messageErr := event.Output.MessageOutput.GetMessage()
		if messageErr != nil {
			return nil, normalizeFrameworkError(messageErr)
		}
