...
type modelClient struct {
	...
	responsesModel      einomodel.AgenticModel
	structuredResponses einomodel.AgenticModel
...
func newModelClientWithTransport(
	...
	if configuration.APIProtocol == domain.ModelAPIProtocolResponses {
		...
		result := &modelClient{configuration: configuration, credential: credential, client: client, logger: logger, diagnostics: diagnostics}
		if err := result.initResponses(); err != nil {
			return nil, capabilityError(domain.ModelErrorCodeInvalidRequest, "model-component")
		}
		return result, nil
	}
	var maximum *int
	...
	newOpenAIComponent := func(responseFormat *einoopenai.ChatCompletionResponseFormat) (einomodel.ToolCallingChatModel, error) {
		...
		if configuration.Temperature != nil {
			fields = map[string]any{"temperature": *configuration.Temperature}
		}
...
// EinoRuntime.Close waits for admitted runs before calling it.
...
func (client *modelClient) streamBounded(
	...
	observeContent func(string) error,
	nativeOptions ...einomodel.Option,
) (*schema.Message, *domain.ModelError) {
	...
	if len(nativeOptions) > 0 {
		stream = schema.StreamReaderWithConvert(stream, func(chunk *schema.Message) (*schema.Message, error) {
			if chunk != nil && adk.GetMessageID(chunk) != "" {
				value := *chunk
				value.Extra = make(map[string]any, len(chunk.Extra))
				for key, item := range chunk.Extra {
					if key != "_eino_msg_id" {
						value.Extra[key] = item
					}
				}
...
func (client *modelClient) generateNonStreaming(
	...
	if client == nil || ctx == nil || (client.model == nil && client.responsesModel == nil) || !requestID.Valid() || len(messages) == 0 ||
		reservation.RequestBytes < 1 || reservation.RequestBytes > domain.MaxModelRequestBytes ||
	...
	if client.responsesModel != nil {
		message, err = client.generateResponses(requestContext, messages, invocation, reservation.OutputBytes)
	} else {
...
func credentialAppearsInBytes(credential *config.SecretValue, value []byte) bool {
	...
	if err := credential.Use(func(secret string) {
		// Native SDK metadata can contain JSON-escaped strings. Check the
		// canonical encoded spelling as well as the decoded credential.
		encoded, err := json.Marshal(secret)
		found = bytes.Contains(value, []byte(secret)) || err != nil ||
			len(encoded) > 2 && bytes.Contains(value, encoded[1:len(encoded)-1])
	}); err != nil {
		return true
	}
	return found
...
func mapHTTPStatus(status int) domain.ModelErrorCode {
	...
	case status == http.StatusBadRequest || status == http.StatusUnprocessableEntity:
		return domain.ModelErrorCodeRequestRejected
...
func failureForModelError(modelError *domain.ModelError, status int) modelFailure {
	...
	case domain.ModelErrorCodeRequestRejected, domain.ModelErrorCodeAuthenticationFailed, domain.ModelErrorCodePermissionDenied,
