...
func TestCoordinatorRepositoryClearHistoryIsAtomicAndPreservesPreferences(t *testing.T) {
	...
	for _, statement := range []string{
		...
		`INSERT INTO privacy_consents (role, policy_version, origin_hash, categories_json, decision, decided_at_ms, schema_version) VALUES ('agent', 'privacy-v1', 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '[]', 'accepted', 12007, 1)`,
	} {
	...
	for table, want := range map[string]int{
		...
		"settings": 2, "privacy_consents": 1, "schema_migrations": 1,
	} {
		var got int
