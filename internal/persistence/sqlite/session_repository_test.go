...
func seedCompleteSessionGraph(t *testing.T, db *DB, rawSessionID string) domain.SessionID {
	...
	statements := []struct {
		...
		{`INSERT INTO model_requests (id, run_id, sequence, provider_kind, model, status, prompt_version, prompt_fingerprint, started_at_ms, finished_at_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, []any{modelID, runID, 1, "openai", "test-model", "succeeded", "prompt-v1", strings.Repeat("1", 64), 1, 2}},
