...
func TestInitialSchemaForeignKeysReferenceOnlyCurrentTables(t *testing.T) {
	db := openTestDB(t, context.Background(), testStateDir(t), "initial-foreign-keys")
	var missing int
	if err := db.handle.GetContext(context.Background(), &missing, `
  SELECT count(fk."table")
  FROM sqlite_schema AS owner, pragma_foreign_key_list(owner.name) AS fk
  WHERE owner.type = 'table'
    AND NOT EXISTS (
      SELECT 1 FROM sqlite_schema AS target
 ...
 `); err != nil {
	...
	if missing != 0 {
		t.Fatalf("Foreign keys with missing targets = %d", missing)
	}
}
...
func TestMigrateRollsBackFailedInitialMigration(t *testing.T) {
	...
	raw := openRawDatabase(t, filepath.Join(stateDir, databaseFilename))
	if _, err := raw.ExecContext(context.Background(), migrationLedgerSQL); err != nil {
		_ = raw.Close()
		t.Fatalf("migration ledger error = %v", err)
	}
...
func assertMigrationRecord(t *testing.T, db *sql.DB, wantApplicationVersion string) {
	...
	defer rows.Close()
	wantNames := []string{"000001_initial.sql"}
	count := 0
	for rows.Next() {
		var version int
		var name string
