...
func TestRunShortCircuitsHelpAndVersion(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "version", args: []string{"version"}, wantContent: "kupilot version=v0.1.0 commit=0123456789ab built=2026-08-08T00:00:00Z go=go1.27.0 platform=linux/arm64\n"},
	}
...
func TestRunPrintsOnlyApprovedSafeStartupErrors(t *testing.T) {
	...
	code := Run(
		...
		func(context.Context, StartIntent) error {
			return safeStartupError{message: "Model API key is required; set KUPILOT_AGENT_API_KEY before starting Kupilot. (model_api_key_missing)"}
		},
	)
	if code != ExitFailure {
	...
	want := "Error: Model API key is required; set KUPILOT_AGENT_API_KEY before starting Kupilot. (model_api_key_missing)\n"
