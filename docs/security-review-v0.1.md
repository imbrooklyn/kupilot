...
| T06: endpoint confusion, redirects, error bodies, or inferred fields exfiltrate credentials, content, or request-policy control | C03 canonical origin, normal TLS, loopback-only HTTP, redirect denial, typed reasoning configuration, consent binding, safe errors, and credential-redacted opt-in diagnostics | `TestValidateModelEndpointPolicy`, `TestRedirectPolicyAllowsOnlyCanonicalOrigin`, `TestPrivateHTTPSUsesCertificateAndHostnameVerification`, `TestCrossOriginRedirectIsDeniedBeforeAuthorizationCanMove`, `TestGPT5CompatibleIdentifierReachesHTTPWithFixedPayload`, `TestHTTPAndTransportErrorCanariesAreBoundedAndDiscarded`, `TestHTTPErrorFixturesMapToSafeClassesWithoutBodyLeakage`, `TestModelRequestErrorMappingUsesObservedHTTPStatusWithoutRetainingRawCause`, sensitive diagnostic redaction tests, and `TestPrivacyConsentLifecycleBindsOriginCategoriesAndPolicy` | **Pass.** Invalid origins fail locally, authorization does not cross origin, the optional reasoning field comes only from exact typed configuration, default logs exclude bodies, explicit sensitive logs remove the exact credential, observed HTTP status survives a generic SDK wrapper, and changed consent tuples prevent content transfer. |
...
| T14: raw vendor errors or logs disclose credentials, bodies, SQL, paths, or cluster data outside explicit sensitive model diagnostics | C01 and C10 safe classifications, default allowlisted logs, and bounded opt-in failure detail | `TestSecurityAssuranceSafeErrorSourceSinkMatrix`, `TestSecurityAssuranceSafeErrorCancellationIdentity`, `TestSecurityAssuranceShutdownAggregationKeepsOnlySafeErrors`, `TestFileLoggerWritesBoundedSafeModelFailureDiagnostics`, `TestFileLoggerWritesOnlyExplicitOptInSensitiveModelDiagnostics`, `TestFileLoggerBoundsOptInSensitiveFields`, model credential-redaction tests, Kubernetes wrapped-error tests, SQLite safe repository errors, and config value-hiding tests | **Pass — SR-004 closed.** Stable classes and default sinks remain content-free; explicit sensitive mode admits only documented bounded model-failure fields, removes the exact credential, and creates no runtime authority. |
...
`TestFileLoggerWritesBoundedSafeModelFailureDiagnostics` separately proves that
the admitted model-failure record retains only fixed metadata and a sink-
generated project-function chain while dropping caller-provided errors, bodies,
stacks, source paths, and generated canaries;
`TestFileLoggerMarksSafeModelCallStackTruncation` proves the fixed chain ceiling
