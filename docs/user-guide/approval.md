# Deployment Restart Approval (`v0.2`)

This guide applies only to a KuPilot composition explicitly identified as
`v0.2`. The `v0.1` composition remains strictly read-only and contains no
approval coordinator, write executor, or Kubernetes write permission.

KuPilot `v0.2` supports one supervised write: `restart_deployment` for one exact
`apps/v1` Deployment in the active Namespace. It changes only the
...
The Agent can propose a restart after a bounded Diagnosis, but it cannot approve
or execute the proposal. The Approval Dialog displays the exact operation,
...
Use the read rules required for diagnosis and add only the exact namespaced
