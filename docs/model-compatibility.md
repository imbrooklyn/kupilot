...
For each bounded model call, the adapter validates a project-owned
`ModelRequest` snapshot, maps the admitted conversation and fixed Tool catalog
to Eino once, and invokes the Eino OpenAI ChatModel directly. Eino serializes
the request, decodes the SSE response, and assembles its message stream. The
adapter then validates one assembled assistant message before any Tool
reservation, Tool dispatch, durable assistant Message, Evidence reference, or
...
envelopes without decoding a second provider protocol. After Eino assembles the
stream, Kupilot allowlists and discards Eino-only metadata, validates the
message shape and finish reason, and applies project policy.
...
finish reason is a bounded no-op and cannot authorize a Tool. Eino assembles
the response before Kupilot resolves its modality. A `stop` response yields
validated final text; `length` yields a local budget stop. For a `tool_calls`
response, any assembled commentary is discarded and only complete indexed Tool
calls proceed to strict binding. Every Tool index must remain in range and the
completed index set must be contiguous from zero.
...
| One assembled assistant response | 128 KiB |
...
| SSE data records or decoded Eino chunks | 1,024, whichever is reached first |
